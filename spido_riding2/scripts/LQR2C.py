...
# == Observer  ======================================================
	...
	while(abs(X)<0.001):
		i=0
	mat=np.loadtxt('/home/summit/Spido_ws/src/spido_riding2/scripts/mat.txt')
	...
	while ((rospy.get_time()-t0<=simul_time)):
		...
		y=C.dot(x)
	        #print('u',u)
        A=get_model_Ac_matrix()
        B=get_model_Bc_matrix(vy,vpsih,cmd.steering_angle_front,cmd.steering_angle_rear)
        C=pinv(Bc)*(L*.dot(xhat2-x2)-Ac*.dot(xhat2))
		A=get_model_A_matrix(k,C(1),C(2))
        B=get_model_B_matrix(C(1),C(2))
