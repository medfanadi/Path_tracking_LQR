...
def Callback(message):
	...
	global X,Y,Psi,ey,epsi,vy,Psip,bf,br,Xref,Yref,Psiref,vpsih,eyy,epsih,bfss
	X=message.data[0]
	Y=message.data[1]
	Psi=message.data[2]
	ey=message.data[3]
...
def recorder():
    ...
    file = open("/home/summit/Spido_ws/recorded_files/single_steering/steerLQRss.txt","w")
    ...
    while (rospy.get_time()-t0<=simul_time):
		...
		file.write(' '.join((str(rospy.get_time()-t0), str(X),str(Y), str(Psi),str(ey),str(epsi), str(vy),str(Psip),str(bf), str(br),str(Xref),str(Yref),str(Psiref),str(vpsih),str(eyy),str(epsih),str(bfss))) +'\n')
		r.sleep()
    file.close()
