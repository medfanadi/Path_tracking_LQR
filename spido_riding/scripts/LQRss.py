...
# == Observer  ======================================================
	...
	while (rospy.get_time()-t0<=simul_time):
		...
		Vyss=-(k*vit*(A[(0,1)]*B[(1,0)] - A[(1,1)]*B[(0,0)] ))/(A[(0,0)]*B[(1,0)] - A[(1,0)]*B[(0,0)])
		Vpsiss=vit*k
		eyss=0;
		epsiss=-Vyss/vit;

		bfss=-(k*vit*(A[(0,0)]*A[(1,1)] - A[(0,1)]*A[(1,0)]))/(A[(0,0)]*B[(1,0)] - A[(1,0)]*B[(0,0)] )
		brss=-bfss;
	...
	#np.array([[-0.5,4.3e+13],[-0.5,-4.2e+13]])#
        ...
        posture = np.array([X,Y,Psi,ey,epsi,vy,Psip,u,xref,yref,Psiref,vpsih,eyy,epsih], dtype=np.float32)
